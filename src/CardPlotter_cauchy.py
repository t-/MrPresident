...
for i in xrange(npairs):
    ...
    fx1 = np.random.standard_cauchy(NSAMPLE)/6.0 + 0.5
    fx2 = np.random.standard_cauchy(NSAMPLE)/6.0 + 0.5
    ...
    plt.ylim((xmin, xmax))
    plt.axis('off')
    plt.savefig('../renderings/find_similar_cauchy%d_1.png' % (i))
    ...
    plt.ylim((xmin, xmax))
    plt.axis('off')
    plt.savefig('../renderings/find_similar_cauchy%d_2.png' % (i))

    hashes.append([a,NSAMPLE])

f = open('../renderings/hash_cauchy.dat', 'w')
for i in hashes:
    print >>f, i
f.close()
